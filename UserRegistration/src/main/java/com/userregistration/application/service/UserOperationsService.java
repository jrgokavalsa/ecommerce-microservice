...
public class UserOperationsService implements UserDetailsService
{
	...
	public ResponseEntity<String> saveUserDetails(UserRegistrationDTO request) {
		...
		if(userRepository.findByEmailIdOrPanCardNoOrAadhaarNo(request.getEmailId(),request.getPanCardNo(),request.getAadhaarNo()).isPresent())
			throw new UserNameAlreadyExistsException("User with details already exists.Please try with another Details");
		...
		strMsg.append("  Use credentials for Login: ").append("User Name:: " + credentials.getUserName())
				.append(" Password:: " + request.getUserName().concat("9281"));
